...
def sequencer_count():
    obj = DNASequencer.objects.all().count()
    if obj == 0:
        return 1
    else:
        return obj + 1
...
class DNASequencer(models.Model):
    ...
    def get_absolute_url(self):
        ...
        return reverse('dna-sequencer-detail', kwargs=kwargs)

    def clean(self, *args, **kwargs):
        # code = self.cleaned_data['product_code']
        pc = DNASequencer.objects.filter(product_code=self.product_code)
        if pc:
            raise ValidationError('Product code already exist!')
        super(DNASequencer, self).clean(*args, **kwargs)
