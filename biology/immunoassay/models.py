...
def immunoassay_count():
    obj = ImmunoAssay.objects.all().count()
    if obj == 0:
        return 1
    else:
        return obj + 1
...
class ImmunoAssay(models.Model):
    ...
    def get_absolute_url(self):
        ...
        return reverse('immunoassay-detail', kwargs=kwargs)

    def clean(self, *args, **kwargs):
        # code = self.cleaned_data['product_code']
        pc = ImmunoAssay.objects.filter(product_code=self.product_code)
        if pc:
            raise ValidationError('Product code already exist!')
        super(ImmunoAssay, self).clean(*args, **kwargs)

    def save(self, *args, **kwargs):
        value = self.name
        self.slug = slugify(value, allow_unicode=True)
        self.product_code = "{}-{}{}{}".format(
            "IS", product_code_start(), immunoassay_count(), product_code_end()
        )
        self.full_clean()
