...
class VR(models.Model):
    ...
    def get_absolute_url(self):
        ...
        return reverse('vr-detail', kwargs=kwargs)

    def clean(self, *args, **kwargs):
        # code = self.cleaned_data['product_code']
        pc = VR.objects.filter(product_code=self.product_code)
        if pc:
            raise ValidationError('Product code already exist!')
        super(VR, self).clean(*args, **kwargs)

    def save(self, *args, **kwargs):
        value = self.name
        self.slug = slugify(value, allow_unicode=True)
        self.product_code = "{}-{}{}{}".format(
            "VR", product_code_start(), vr_count(), product_code_end()
        )
        self.full_clean()
