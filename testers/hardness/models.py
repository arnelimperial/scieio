...
def h_count():
    obj = Hardness.objects.all().count()
    if obj == 0:
        return 1
    else:
        return obj + 1
...
class Hardness(models.Model):
    tester = models.ForeignKey(TestSemiConductor, on_delete=models.CASCADE)
    ...
    def get_absolute_url(self):
        ...
        return reverse('hardness-detail', kwargs=kwargs)

    def clean(self, *args, **kwargs):
        # code = self.cleaned_data['product_code']
        pc = Hardness.objects.filter(product_code=self.product_code)
        if pc:
            raise ValidationError('Product code already exist!')
        super(Hardness, self).clean(*args, **kwargs)

    def save(self, *args, **kwargs):
        value = self.name
        self.slug = slugify(value, allow_unicode=True)
        self.product_code = "{}-{}{}{}".format(
            "H", product_code_start(), h_count(), product_code_end()
        )
        self.full_clean()
