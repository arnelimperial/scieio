...
def wt_count():
    obj = WaterTreatment.objects.all().count()
    if obj == 0:
        return 1
    else:
        return obj + 1
...
class WaterTreatment(models.Model):
    ...
    def get_absolute_url(self):
        ...
        return reverse('water-treatment-detail', kwargs=kwargs)

    def clean(self, *args, **kwargs):
        # code = self.cleaned_data['product_code']
        pc = WaterTreatment.objects.filter(product_code=self.product_code)
        if pc:
            raise ValidationError('Product code already exist!')
        super(WaterTreatment, self).clean(*args, **kwargs)

    def save(self, *args, **kwargs):
        value = self.name
        self.slug = slugify(value, allow_unicode=True)
        self.product_code = "{}-{}{}{}".format(
            "WTE", product_code_start(), wt_count(), product_code_end()
        )
        self.full_clean()
